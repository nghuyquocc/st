...
{
	float winx, winy, xp, yp, cluster_xp, cluster_yp, runewidth;
	...
	/* Initial values and metrics */
	xresetfontsettings(mode, &font, &frcflags);
	winx = win.hborderpx + x * win.cw;
	winy = win.vborderpx + y * win.ch;
	runewidth = win.cw * ((glyphs[0].mode & ATTR_WIDE) ? 2.0f : 1.0f);

	/* Shape the segment */
	HbTransformData shaped = (HbTransformData){0};
	hbtransform(&shaped, font->match, glyphs, 0, len);

	/* Starting pen position for the cluster */
	xp = winx;
	yp = winy + font->ascent;
	cluster_xp = xp;
	cluster_yp = yp;

	for (code_idx = 0; code_idx < (int)shaped.count && numspecs < len; code_idx++) {
		int idx = shaped.glyphs[code_idx].cluster;

		/* Skip dummy cells (second half of wide chars) */
		if (glyphs[idx].mode & ATTR_WDUMMY)
			continue;

		/* New cluster? move the pen to the next cell */
		if (code_idx > 0 && idx != (int)shaped.glyphs[code_idx - 1].cluster) {
			xp += runewidth;
			cluster_xp = xp;
			cluster_yp = yp;
		}
		...
		if (shaped.glyphs[code_idx].codepoint != 0) {
			/* HarfBuzz provided a glyph: emit with HB positions */
			...
			/* HB returned no glyph; fallback to font cache / fontconfig */
			rune = glyphs[idx].u;

			/* Try existing fallback cache first */
			for (f = 0; f < frclen; f++) {
				glyphidx = XftCharIndex(xw.dpy, frc[f].font, rune);
				if ((glyphidx && frc[f].flags == frcflags) ||
				    (!glyphidx && frc[f].flags == frcflags && frc[f].unicodep == rune))
					break;
			}

			/* Not in cache: ask fontconfig for a font that can render the rune */
			if (f >= frclen) {
				if (!font->set)
			...
			/* Emit fallback glyph at the cell origin (no HB positioning) */
